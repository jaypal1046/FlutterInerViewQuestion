...
async function getTickets() {
    const res = await fetch("https://jaypal1046.pythonanywhere.com/Getcollection/3/Flutter",{
        next:{
  ...
  export default async function FatchAdvanceList() {
    ...
    return (
      <>
        {flutterQ["data"].map((flutters) => (
          ...
          <div key={`advance ${flutters.QID}`} className="card my-5">
            <h3>Question: {flutters.question}</h3>
            <p>Amswere: {flutters.answer}</p>
            <div className={`pill advance ${flutters.difficulty}`}>{flutters.difficulty}</div>
          </div>
          </Link>
        ))}
