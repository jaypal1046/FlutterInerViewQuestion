...
async function getTickets() {
    const res = await fetch("https://jaypal1046.pythonanywhere.com/Getcollection/0/Flutter",{
        next:{
  ...
  export default async function FatchList() {
      ...
      <>
        {flutterQ["data"].map((flutters) => (
          ...
          <div key={flutters.QID} className="card my-5">
            ...
            <div className={`pill ${flutters.difficulty}`}>{flutters.difficulty}</div>
          </div>
          </Link>
        ))}
